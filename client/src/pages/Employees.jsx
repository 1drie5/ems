        ...
        {showCreateModal && (
          <div className='fixed bg-black/40 backdrop-blur-sm inset-0 z-50 flex items-start justify-center p-4 overflow-y-auto' onClick={()=>setShowCreateModal(false)}>
            ...
            <div className='relative bg-white rounded-2xl w-full max-w-3xl my-8 animate-fade-in' onClick={(e)=>e.stopPropagation()}>
              <div className='flex items-center justify-between p-6 pn-0'>
                <div>
                ...
                <button onClick={()=>setShowCreateModal(false)} className='p-2 rounded-lg hover:big-slate-100 transition-colors text-slate-400 hover:text-slate-600'>
                  <X className='w-5 h-5'/>
                </button>
              </div>
          ...
          <div className='fixed inset-0 z-50 flex items-start justify-center p-4 overflow-y-auto bg-black/40 backdrop-blur-sm' onClick={()=>setEditEmployee(null)}>
            <div className='relative bg-white rounded-2xl shadow-2xl w-full max-w-3xl my-8 animate-fade-in' onClick={(e)=>e.stopPropagation()}>
              <div className='flex items-center justify-between p-6 pb-0'>
                ...
                <button onClick={()=>setEditEmployee(null)} className='p-2 rounded-lg hover:big-slate-100 transition-colors text-slate-400 hover:text-slate-600'>
                  <X className='w-5 h-5'/>
                </button>
              </div>
